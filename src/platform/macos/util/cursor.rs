...
impl From<MouseCursor> for Cursor {
    fn from(cursor: MouseCursor) -> Self {
        match cursor {
            ...
            MouseCursor::NotAllowed | MouseCursor::NoDrop => {
            ...
            MouseCursor::EwResize | MouseCursor::ColResize => {
                Cursor::Native("resizeLeftRightCursor")
            }
            ...
            MouseCursor::NeswResize => {
                Cursor::Undocumented("_windowResizeNorthEastSouthWestCursor")
            }
            MouseCursor::NwseResize => {
                Cursor::Undocumented("_windowResizeNorthWestSouthEastCursor")
            }
            ...
            MouseCursor::Wait | MouseCursor::Progress => {
    ...
    pub unsafe fn load(&self) -> id {
        match self {
            ...
            Cursor::WebKit(cursor_name) => {
                load_webkit_cursor(cursor_name).unwrap_or_else(|message| {
                    warn!("{}", message);
                    Self::default().load()
                })
            }
...
unsafe fn load_webkit_cursor(cursor_name_str: &str) -> Result<id, String> {
    ...
    let image = NSImage::alloc(nil)
        ...
        .ok_or_else(|| format!("Failed to read image for `{}` cursor", cursor_name_str))?;
    let info = NSDictionary::dictionaryWithContentsOfFile_(nil, info_path)
        .into_option()
        .ok_or_else(|| format!("Failed to read info for `{}` cursor", cursor_name_str))?;
    ...
    cursor
        .into_option()
        .ok_or_else(|| format!("Failed to initialize `{}` cursor", cursor_name_str))
}
